...
class UltraFastVideoStitcher:
    ...
    def show_stitched_video_optimized(self):
        ...
        if hasattr(self, 'use_roi') and self.use_roi:
            ...
            print(f"   ROI 크기: {self.roi_width} x {self.roi_height}")
            print(f"   예상 FPS 향상: ROI 크기 감소로 인한 추가 20-30% 향상")
        else:
            print(f"🎯 ROI 모드: 비활성화 (전체 화면)")
        
        print(f"🚀 적용된 최적화: JSON 단일로딩 + 캔버스캐시 + 조건문제거 + 마스크사전계산")
        print(f"👁️ 아이트래킹: YOLO + MediaPipe FaceMesh + 시선 안정화")
        ...
        try:
            while True:
                ...
                # 🚀 시선 추적 정보 표시
                self.draw_gaze_info(stitched_display, gaze_data, self.current_fps)
                ...
                # 🆕 ROI 정보 표시
                if hasattr(self, 'use_roi') and self.use_roi:
                    roi_text = f"ROI: ({self.roi_x1},{self.roi_y1}) -> ({self.roi_x2},{self.roi_y2})"
                    cv2.putText(stitched_display, roi_text, (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                    roi_size_text = f"ROI Size: {self.roi_width} x {self.roi_height}"
                    cv2.putText(stitched_display, roi_size_text, (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
...
# 🚀 초고속 최적화된 실시간 스티칭 실행
    print("🚀 Ultra-Fast Video Stitcher + Eye Tracking v4.0 시작")
    print("예상 성능 향상: 10 FPS → 25-35 FPS (300-400% 향상)")
    print("👁️ 아이트래킹: YOLO 얼굴 검출 + MediaPipe FaceMesh + 시선 안정화")
    ...
    if hasattr(stitcher, 'use_roi') and stitcher.use_roi:
        ...
        print("   🚀 ROI 기반 최적화로 추가 FPS 향상 예상")
    else:
