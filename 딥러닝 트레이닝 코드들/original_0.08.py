...
# ---------------------------
def rmse_sum_per_channel(pred: torch.Tensor, tgt: torch.Tensor):
    ...
    details["rmse_sum"] = float(loss.detach().cpu())
    return loss, details
...
def train(cfg: TrainConfig):
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer, max_lr=cfg.lr, epochs=cfg.epochs, steps_per_epoch=steps,
        pct_start=cfg.pct_start, div_factor=25.0, final_div_factor=100.0, anneal_strategy="cos"
    )
...
if __name__ == "__main__":
    ...
    cfg = TrainConfig(
        ...
        batch_size=8,                                    # 논문과 동일한 작은 배치 (32 → 8)
        epochs=80,
        lr=1e-3,                                         # 논문과 동일한 학습률 (1e-4 → 1e-3)
        weight_decay=1e-4,                               # 논문과 동일한 가중치 감쇠 (1e-2 → 1e-4)
        max_grad_norm=1.0,                               # 표준 그래디언트 클리핑 (0.5 → 1.0)
        pct_start=0.1,                                   # 표준 OneCycleLR 설정 (0.5 → 0.1)
        num_workers=4,
        amp=True,
        seed=42,
        patience=12,                                     # 표준 인내심 (20 → 12)
        save_path="finch_3head_90_180_270.pth",   # ⛳️ 여기에 .pth 저장됩니다
    )
    print(cfg)
    train(cfg)
